  ...
  methods: {
    scrapeIds (list) {
      this.itemList = list.map(item => {
        return {id: item.url.slice(34).replace('/', '').padStart(3, '0'), name: item.name}
      })
    },
    getPage (offset, caller = null) {
      if (caller !== null) {
        caller === 'buttonNext' ? this.button.nextActive = true : this.button.prevActive = true
      }
      fetch(this.hostname + this.param + '&offset=' + offset)
        ...
        .then(data => {
          ...
          if (caller !== null) {
            caller === 'buttonNext' ? this.button.nextActive = false : this.button.prevActive = false
          }
        })
    },
    nextPage () {
